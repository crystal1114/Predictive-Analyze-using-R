...
setwd("/Users/shutingchen/Desktop/USC/ISE 529               Data Analytics/project/data science with r/titanic")
# Load raw data
...
#50% of data are less than 14.454, but the mean of data is almost double of that value 
#which means that the data is skewed to the high end. long tail distribution
#sometimes even if you have a numeric variable if there is small number
#of unique values in you dataset you could transfer it into factor
...
#video 4
#pre-process
#some agrithm are only used for numeric vars not for factor or categorial
#use oob-the rows that are not being selected to test the accuracy of tree
...
#we do good when people perish we predict them perish, but not so good when they survived
#we are too pessimistic about that, the reason might be that the data is skewed

table(train$survived)
#  0   1 
#549 342 
varImpPlot(rf.1)
#see from picture which factor is a good predictor
#understanding the data is the first thing
